// igual ao 2 com adicao do break e da checagem de botao E estado
...
void loop() {
  if (estado == 1) { // checa o estado
    for (int i = 0; i < 3; i++) { // loop pra percorrer o vertor ordem
      if (digitalRead(botao1) == LOW && estado == 1) { // checa o botao e o estado
        estado = 2;
        break;
      }
      else if (digitalRead(botao2) == LOW && estado == 2) { // checa o outro botao e o outro estado
        estado = 1;
        break; // "quebra" o loop for, voltando ao void loop
      }
  ...
  if (estado == 2) { // mesma coisa que o de cima, mudando apenas a ordem
    for (int i = 0; i < 3; i++) {
      ...
      else if (digitalRead(botao2) == LOW && estado == 2) {
        estado = 1;
        break;
      }
